...
{
    public static final ResourceLocation DEFAULT_TYPE = ResourceLocation.fromNamespaceAndPath("auralithpioneerinitiative", "main_sequence");

    private static SunDefinition fromCodec(ResourceLocation texture, float size, float axialRotationSpeed, List<Float> glowColor, int glowLayers, float glowScale, ResourceLocation type, Optional<JetConeDefinition> jetCone, Optional<BlackHoleDefinition> blackHole)
    {
        return new SunDefinition(texture, size, axialRotationSpeed, glowColor, glowLayers, glowScale, type, jetCone.orElse(null), blackHole.orElse(null));
    }

    public static final Codec<SunDefinition> CODEC = RecordCodecBuilder.create(instance ->
        instance.group(
            ResourceLocation.CODEC.fieldOf("texture").forGetter(SunDefinition::texture),
