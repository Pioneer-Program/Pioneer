...
public final class StarTypeRegistry
{
    ...
    public static void register(ResourceLocation id, StarTypeRenderer renderer)
    {
        ...
        if (existing != null) Auralithpioneerinitiative.LOGGER.warn("[Auralith] Star type '{}' registered twice — keeping the first registration", id);
    }
    ...
    public static StarTypeRenderer get(ResourceLocation id)
    {
        ...
        if (renderer == null)
        {
            Auralithpioneerinitiative.LOGGER.warn("[Auralith] No star type registered for '{}', falling back to main_sequence (registered ids: {})", id, TYPES.keySet());
