...
public final class TextureManager<G>
{
    ...
    private void evictIfNeeded()
    {
        ...
        while (cache.size() > maxIdleEntries && it.hasNext())
        {
            ...
            closeEntry(entry);
            it.remove();
            Auralithpioneerinitiative.LOGGER.debug("[Auralith] TextureManager evicted '{}' ({} entries)", mapEntry.getKey(), cache.size());
        }
    }
    ...
    private static ResourceLocation register(NativeImage image, String groupKey, String faceName)
    {
        ...
        ResourceLocation rl = ResourceLocation.fromNamespaceAndPath(Auralithpioneerinitiative.MODID, safeName);
