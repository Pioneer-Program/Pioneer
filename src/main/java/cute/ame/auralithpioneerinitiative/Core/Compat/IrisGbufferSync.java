...
public final class IrisGbufferSync
{
    ...
    private static synchronized void init()
    {
        ...
        try
        {
            ...
            available = true;
            Auralithpioneerinitiative.LOGGER.info("Iris gbuffer sync hook initialized");
        } catch (Throwable t)
        {
            available = false;
            Auralithpioneerinitiative.LOGGER.debug("Iris gbuffer sync hook unavailable ({}) :3", t.toString());
        }
    }
    ...
    public static void syncGbufferTargetIfNeeded()
    {
        ...
        {
            Auralithpioneerinitiative.LOGGER.warn("failed to sync Iris gbuffer target for custom shader draw :cccc", t);
        }
    }
}
