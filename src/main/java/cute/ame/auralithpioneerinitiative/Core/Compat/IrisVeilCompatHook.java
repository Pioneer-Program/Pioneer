...
public final class IrisVeilCompatHook
{
    ...
    private static synchronized void init()
    {
        ...
        {
            Auralithpioneerinitiative.LOGGER.debug("iris-veil-compat not present, skipping shader exclusion hook");
            available = false;
        }
        catch (ReflectiveOperationException e)
        {
            Auralithpioneerinitiative.LOGGER.warn("Failed to bind iris-veil-compat exclusion API", e);
            available = false;
        }
    }
    ...
    public static void tryExcludeShaderReplacement(ResourceLocation shaderPath)
    {
        ...
        {
            Auralithpioneerinitiative.LOGGER.warn("Failed to register shader exclusion for '{}' with iris-veil-compat", shaderPath, t);
        }
    }
    ...
    public static void tryExcludeAuralithVfxShaders()
    {
        ...
        if (available) Auralithpioneerinitiative.LOGGER.info("Registered {} VFX shader(s) with iris-veil-compat exclusion list", count);
    }
}
