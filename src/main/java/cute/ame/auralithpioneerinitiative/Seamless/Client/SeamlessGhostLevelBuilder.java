...
public final class SeamlessGhostLevelBuilder
{
    ...
    private static ClientLevel tryCreateGhostLevel(ResourceKey<Level> target, ClientPacketListener connection)
    {
        ...
        if (dimTypeKey == null)
        {
            Auralithpioneerinitiative.LOGGER.warn("[Auralith] No known dimension_type for {}, can't build ghost level yet (preload hint missing/late?)", target.location());
            return null;
        }
        ...
        try
        {
            ...
            Auralithpioneerinitiative.LOGGER.debug("[Auralith] Built static ghost ClientLevel for {} (dimension_type={})", target.location(), dimTypeKey.location());
            return ghost;
        }
        catch (Exception e)
        {
            Auralithpioneerinitiative.LOGGER.warn("[Auralith] Failed to construct ghost ClientLevel for {}: {}", target.location(), e.toString());
            return null;
        }
    }
    ...
    private static void injectChunk(ClientLevel ghost, ResourceKey<Level> target, ChunkPos pos, byte[] bytes)
    {
        try
        {
            ...
            Auralithpioneerinitiative.LOGGER.debug("[Auralith] Injected ghost chunk {} into static level {}", pos, target.location());
            if (!ghost.hasChunk(pos.x, pos.z)) Auralithpioneerinitiative.LOGGER.warn("[Auralith] Ghost chunk {} for {} reported injected but is NOT present in the level afterward (likely dropped by ClientChunkCache view range)", pos, target.location());
        }
        catch (Exception e)
        {
            Auralithpioneerinitiative.LOGGER.warn("[Auralith] Failed to inject ghost chunk {} for {}: {}", pos, target.location(), e.toString());
        }
    }
