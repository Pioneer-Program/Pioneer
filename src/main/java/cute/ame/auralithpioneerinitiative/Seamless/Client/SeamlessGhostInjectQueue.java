...
public final class SeamlessGhostInjectQueue
{
    ...
    public static void drain()
    {
        ...
        while (!QUEUE.isEmpty() && System.nanoTime() < deadline)
        {
            ...
            {
                Auralithpioneerinitiative.LOGGER.warn("[Auralith] Queued ghost chunk {} failed for {} in {}: {}", p.kind(), p.pos(), p.target().location(), e.toString());
            }
            processed++;
        }

        if (processed > 0) Auralithpioneerinitiative.LOGGER.debug("[Auralith] Drained {} queued ghost chunk task(s), {} remaining", processed, QUEUE.size());
    }
