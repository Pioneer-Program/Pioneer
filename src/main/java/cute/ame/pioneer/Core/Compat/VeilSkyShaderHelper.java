...
public final class VeilSkyShaderHelper
{
    ...
    public static void draw(ResourceLocation shaderPath, Consumer<ShaderProgram> uniformSetup, Consumer<RenderType> geometryAndDraw)
    {
        ...
        if (renderType == null)
        {
            if (WARNED.add(shaderPath)) Pioneer.LOGGER.warn("VeilRenderType.get({}) returned null, render type JSON missing/failed to parse, drawing nothing T*T", shaderPath);
            return;
        }
        ...
        if (shader == null)
        {
            if (WARNED.add(ResourceLocation.fromNamespaceAndPath(shaderPath.getNamespace(), shaderPath.getPath() + "_shadernull"))) Pioneer.LOGGER.warn("VeilRenderSystem.setShader({}) returned null, shader program failed to compile/resolve.. skipping draw this frame :c", shaderPath);
            return;
        }
